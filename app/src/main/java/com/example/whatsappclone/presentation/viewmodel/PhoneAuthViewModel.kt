...
class PhoneAuthViewModel @Inject constructor(
    ...
    private fun signInWithCredential(credential: PhoneAuthCredential, context: Context) {
        ...
        firebaseAuth.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                _authState.value =
                    com.example.whatsappclone.presentation.viewmodel.AuthState.Success(
                        phoneAuthUser
                    )
    ...
    fun verifyCode(otp: String, context: Context) {
        ...
        if (currentAuthState !is com.example.whatsappclone.presentation.viewmodel.AuthState.CodeSent || currentAuthState.verificationId.isEmpty()) {
            Log.e("PhoneAuth", "Attempting to verify OTP without a valid verification ID")
            _authState.value =
                com.example.whatsappclone.presentation.viewmodel.AuthState.Error("Verification not started or invalid ID")
            return
        }
    ...
    fun convertBitmapTpBase64(bitmap: Bitmap): String {
